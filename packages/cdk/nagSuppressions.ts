...
export const nagSuppressions = (stack: Stack) => {
  ...
  // Add CFN Guard metadata suppressions for Lambda permissions
  // These are auto-generated by CDK for S3 bucket notifications and API Gateway integrations
  const lambdaPermissions = stack.node.findAll().filter(node =>
    node.node.id.includes("AllowBucketNotifications") ||
    node.node.id.includes("ApiPermission")
  )

  lambdaPermissions.forEach(permission => {
    if (permission instanceof CfnResource && permission.cfnResourceType === "AWS::Lambda::Permission") {
      permission.addMetadata("guard", {
        SuppressedRules: ["LAMBDA_FUNCTION_PUBLIC_ACCESS_PROHIBITED"]
      })
    }
  })
